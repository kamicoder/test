...
def git_pull_in_directories(root_dir):
    for path in Path(root_dir).rglob('*.git'):
        ...
        try:
            ...
            process = subprocess.Popen(['git', 'pull'], cwd=parent_dir, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True)
            ...
            if process.returncode != 0:
                print(f"Erreur lors de l'exécution de 'git pull' dans {parent_dir}")
